...
def _c4a4_get_ent_presets():
    global ent_presets
    if ent_presets is None:
        from . import ent_presets as _module
        ent_presets = _module
    return ent_presets
...
def _spawn_jet(xr, zr, ar, sr, yawr, pr, rr, omega_max_dps):
    ...
    jet = _c4a4_get_ent_presets().create_Sukoi57(p, v, R)
...
def _spawn_missile_random(jet):
    ...
    m = _c4a4_get_ent_presets().create_PAC3(starting_position=pos, starting_velocity=vel, target_entity=jet)
    try:
...
# === Q1-C4A4 CANDIDATE SCENARIO RNG STATE CONTRACT ===
import copy as _c4a4_copy


def configure_scenario_seed(seed: int) -> None:
    global _rng
    _rng = np.random.default_rng(int(seed))


def get_scenario_rng_state():
    return _c4a4_copy.deepcopy(_rng.bit_generator.state)


def set_scenario_rng_state(state) -> None:
    global _rng
    generator = np.random.default_rng()
    generator.bit_generator.state = _c4a4_copy.deepcopy(state)
    _rng = generator


def scenario_rng_probe_draw(count: int = 4):
    return _rng.random(int(count)).tolist()

# === END Q1-C4A4 CANDIDATE SCENARIO RNG STATE CONTRACT ===
